...
test_that("ddbs_read_table reads a table into an sf object", {
  ...
  result <- ddbs_read_table(conn_test, "test_points_read", crs = 4326)
...
test_that("ddbs_read_table reads an Arrow view into an sf object", {
  ...
  result <- ddbs_read_table(conn_test, "test_points_view_read", crs = 4326)
...
test_that("ddbs_read_table reads a standard SQL view into an sf object", {
  ...
  result <- ddbs_read_table(conn_test, "test_sql_view", crs = 4326)
...
test_that("ddbs_read_table's 'clauses' argument works", {
  ...
  result <- ddbs_read_table(conn_test, "test_points_read", crs = 4326, clauses = "WHERE id <= 3")
...
test_that("ddbs_read_table works with different geometry types", {
  # Test with polygons
  result <- ddbs_read_table(conn_test, "test_countries_read", crs = 4326)
  expect_s3_class(result, "sf")
...
test_that("ddbs_read_table 'crs' parameter overrides stored CRS", {
  # Store with 4326, read with a different one (e.g., 3857)
  result <- ddbs_read_table(conn_test, "test_points_read", crs = 3857)
...
test_that("ddbs_read_table handles tables with no CRS info", {
  ...
  result_with_crs <- ddbs_read_table(conn_test, "test_no_crs_read", crs = 4326)
