...
Once you have a connection, write spatial data into it with `ddbs_write_vector()`. It accepts both `sf` and `duckspatial_df` objects:
...
`ddbs_read_vector()` materialises a table directly as `sf` (not lazily), so the result can be passed straight to `plot()`:
