...
precision. Coordinates are rounded to the nearest multiple of
`precision` (e.g. `precision = 0.1` keeps one decimal place,
`precision = 1` snaps to integers). The CRS is preserved.
...
- precision:
  ...
  Numeric grid size to snap coordinates to. Smaller values keep more
  precision (e.g. `0.001`); larger values snap more aggressively (e.g.
  `1` snaps to integers).
