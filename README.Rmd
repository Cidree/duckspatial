...
In this case, we can see that DuckDB was `r round(elapsed_gpkg / elapsed_duckdb, 1)` times faster. We can also take advantage of the Arrow-backend database view by using the argument `temp_view = TRUE`:
...
This was `r round(elapsed_gpkg / elapsed_duckdb_view, 1)` times faster than `sf`, and `r round(elapsed_duckdb / elapsed_duckdb_view, 1)` times faster than create a table in DuckDB.
