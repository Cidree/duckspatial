...
ddbs_exterior_ring <- function(
    ...
    if (!is.null(name)) {
        ...
        if (length(x_rest) == 0) {
            tmp.query <- glue::glue("
            SELECT ST_ExteriorRing({x_geom}) as {x_geom}
            FROM {x_list$query_name};
        ")
        } else {
            tmp.query <- glue::glue("
    ...
    if (length(x_rest) == 0) {
        tmp.query <- glue::glue("
            SELECT ST_AsWKB(ST_ExteriorRing({x_geom})) as {x_geom}
            FROM {x_list$query_name};
        ")
    } else {
        tmp.query <- glue::glue("
...
ddbs_make_polygon <- function(
    ...
    if (!is.null(name)) {
        ...
        if (length(x_rest) == 0) {
            tmp.query <- glue::glue("
            SELECT ST_MakePolygon({x_geom}) as {x_geom}
            FROM {x_list$query_name};
        ")
        } else {
            tmp.query <- glue::glue("
    ...
    if (length(x_rest) == 0) {
        tmp.query <- glue::glue("
            SELECT ST_AsWKB(ST_MakePolygon({x_geom})) as {x_geom}
            FROM {x_list$query_name};
        ")
    } else {
        tmp.query <- glue::glue("
